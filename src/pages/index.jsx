...
  
  // Estados para o Modal de Cotas Supabase
  const [showCotasModal, setShowCotasModal] = useState(false);
...
  // Busca do Supabase ao clicar no botão
  const handleOpenCotasModal = async () => {
    setShowCotasModal(true);
    setCotasData({ menorCota: null, maiorCota: null, loading: true, error: null });

    try {
      const res = await fetch('/api/cotas/maior-menor');
      const data = await res.json();

      if (res.ok) {
        setCotasData({
          menorCota: data.menorCota,
          maiorCota: data.maiorCota,
          loading: false,
          error: null
        });
      } else {
        throw new Error(data.message || 'Erro ao carregar do Supabase');
      }
    } catch (err) {
      setCotasData({
        menorCota: null,
        maiorCota: null,
        loading: false,
        error: 'Erro ao buscar cotas no banco de dados.'
      });
    }
  };
...
          0% { opacity: 0; transform: translateY(-10px); background-color: rgba(34, 197, 94, 0.25); }
          50% { background-color: rgba(34, 197, 94, 0.15); }
          100% { opacity: 1; transform: translateY(0); background-color: transparent; }
        }
        .animate-purchase { animation: fadeInSlide 0.6s ease-out forwards; }

        @keyframes pulseGlow {
          0% { box-shadow: 0 0 10px rgba(245, 158, 11, 0.4); transform: scale(1); }
          50% { box-shadow: 0 0 22px rgba(245, 158, 11, 0.85); transform: scale(1.015); }
          100% { box-shadow: 0 0 10px rgba(245, 158, 11, 0.4); transform: scale(1); }
        }
...
        }
        .bonus-banner-content { background-color: #0f172a; border-radius: 14px; padding: 12px 14px; text-align: center; }
        .bonus-title { display: flex; align-items: center; justify-content: center; gap: 8px; color: #fbbf24; font-weight: 900; font-size: 0.85rem; letter-spacing: 0.5px; text-transform: uppercase; }
        .bonus-banner-content p { color: #e2e8f0; font-size: 0.8rem; margin: 6px 0; line-height: 1.4; }
        .highlight-bonus { color: #22c55e; font-weight: 900; font-size: 0.95rem; }
        .bonus-badge { display: inline-flex; align-items: center; gap: 6px; background: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.3); padding: 3px 12px; border-radius: 20px; font-size: 0.7rem; color: #fcd34d; font-weight: 700; margin-top: 2px; text-transform: uppercase; }
        .badge-dot { width: 6px; height: 6px; background-color: #22c55e; border-radius: 50%; display: inline-block; }
      `}</style>
...
            style={{ width: '71px', height: '71px', borderRadius: '50%', objectFit: 'cover', border: '1px solid #fbbf24' }} 
          />
          <div>
            <h1 style={{ fontSize: '1.2rem', fontWeight: '900' }}>SEGUIDOR</h1>
            <span style={{ fontSize: '0.65rem', color: '#fbbf24' }}>★ DE PRÊMIOS ★</span>
          </div>
        </div>
        
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          <button 
            onClick={handleOpenCotasModal}
            style={{ background: '#059669', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '4px' }}
          >
            🏆 Maior/Menor Cota
          </button>
...
            style={{ background: '#1e293b', color: '#38bdf8', border: '1px solid #334155', padding: '6px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer' }}
          >
            🔍 Meus Números
          </button>
          
...
            style={{ background: 'transparent', border: '1px solid #334155', padding: '6px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', color: '#ffffff', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }}
          >
            💬 Fale Conosco
          </a>
          
...
            target="_blank"
            rel="noopener noreferrer"
            style={{ background: '#16a34a', border: 'none', color: '#fff', padding: '6px 14px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer', textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}
          >
            🟢 Quem somos!
          </a>
        </div>
...
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', fontSize: '0.8rem', borderRadius: '6px', transition: 'all 0.3s ease' }}>
...
        {/* Estatísticas e Botões de Seleção */}
...
          <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid #f59e0b', borderRadius: '12px', padding: '12px 14px', marginBottom: '20px', textAlign: 'center' }}>
...
        {/* Footer */}
        <footer style={{ marginTop: '30px', padding: '24px 16px', borderTop: '1px solid #1e293b', fontSize: '0.75rem', color: '#94a3b8', textAlign: 'center', lineHeight: '1.6' }}>
...
            Este bilhete de loteria está autorizado com base na portaria AUTORIZAÇÃO LOTEP. Confira o resultado em <strong>www.seguidordepremios.com.br</strong>
          </p>
...
      {/* MODAL POP-UP MAIOR E MENOR COTA (SUPABASE) */}
      {showCotasModal && (
        <div style={{
...
            width: '100%', maxWidth: '480px', padding: '20px', position: 'relative',
            boxSizing: 'border-box', color: '#fff', boxShadow: '0 10px 30px rgba(0,0,0,0.8)'
          }}>
            
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '1.4rem' }}>🏆</span>
                <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 'bold' }}>Resultado Maior e Menor Cota</h3>
              </div>
              <button 
                onClick={() => setShowCotasModal(false)}
...
                  <span style={{ fontSize: '3.5rem', fontWeight: '900', color: '#fff', lineHeight: 1 }}>1</span>
                  <div style={{ flex: 1, textAlign: 'center' }}>
                    <h4 style={{ margin: '0 0 4px', fontSize: '0.95rem', letterSpacing: '0.5px' }}>MENOR COTA</h4>
                    <p style={{ margin: '0 0 6px', fontSize: '0.8rem', color: '#9ca3af' }}>
                      Nome: {cotasData.menorCota?.nome || 'Nenhuma compra'}
                    </p>
                    <div style={{ background: '#059669', color: '#fff', padding: '6px', borderRadius: '12px', fontWeight: 'bold', fontSize: '0.85rem', marginBottom: '6px' }}>
                      Título: {cotasData.menorCota?.numero || '------'}
                    </div>
                    <p style={{ margin: '2px 0', fontSize: '0.75rem', color: '#9ca3af' }}>
                      Telefone: {cotasData.menorCota?.telefone || '---'}
                    </p>
                    <p style={{ margin: '2px 0', fontSize: '0.75rem', color: '#9ca3af' }}>
                      Data/Hora: {cotasData.menorCota?.dataHora || '---'}
                    </p>
                  </div>
                </div>
...
                  <span style={{ fontSize: '3.5rem', fontWeight: '900', color: '#fff', lineHeight: 1 }}>1</span>
                  <div style={{ flex: 1, textAlign: 'center' }}>
                    <h4 style={{ margin: '0 0 4px', fontSize: '0.95rem', letterSpacing: '0.5px' }}>MAIOR COTA</h4>
                    <p style={{ margin: '0 0 6px', fontSize: '0.8rem', color: '#9ca3af' }}>
                      Nome: {cotasData.maiorCota?.nome || 'Nenhuma compra'}
                    </p>
                    <div style={{ background: '#059669', color: '#fff', padding: '6px', borderRadius: '12px', fontWeight: 'bold', fontSize: '0.85rem', marginBottom: '6px' }}>
                      Título: {cotasData.maiorCota?.numero || '------'}
                    </div>
                    <p style={{ margin: '2px 0', fontSize: '0.75rem', color: '#9ca3af' }}>
                      Telefone: {cotasData.maiorCota?.telefone || '---'}
                    </p>
                    <p style={{ margin: '2px 0', fontSize: '0.75rem', color: '#9ca3af' }}>
                      Data/Hora: {cotasData.maiorCota?.dataHora || '---'}
                    </p>
                  </div>
                </div>
              </>
...
            <button 
              onClick={() => setShowCotasModal(false)}
              style={{ width: '100%', background: '#4b5563', color: '#fff', border: 'none', padding: '10px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', marginTop: '10px' }}>
              Fechar
            </button>
          </div>
        </div>
