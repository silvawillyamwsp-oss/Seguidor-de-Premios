...
export default function MeusNumeros() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`/api/my-tickets?phone=${cleanPhone}`);
      ...
      console.error(err);
      setErrorMsg('Erro de conexão ao buscar cotas. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSearch} style={{ marginBottom: '28px' }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '10px',
              border: 'none',
              background: loading ? '#0284c7' : '#0284c7',
              color: '#fff',
              fontSize: '1rem',
              fontWeight: '900',
            ...
            {results.tickets && results.tickets.length > 0 ? (
              ...
              <div style={{ textAlign: 'center', padding: '12px 0' }}>
                ...
                <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>
                  Não encontramos compras associadas a esse telefone. Verifique se o DDD e o número foram digitados corretamente.
                </p>
              </div>
            )}
          </div>
